...
for i in file_list:
    ...
    try:
        ...
        print("Before Change")
        print(dst)
        print_wavfile_info(dst)
        if wav.frame_rate != 16000:
            wavfile_sampling_change(dst, 16000) #44.1 khz --> 16 khz 로 변경
        
        if wav.sample_width*8 !=16:
            wavfile_bitrate_change(dst, 2) 
    
        if int(wav.frame_width/wav.sample_width)==2:
            wavfile_channel_change(dst, 1)# stereo --> mono 변경
    
        print("After %d Change"%(k))
        k+=1
        print_wavfile_info(dst)
    except:
        print("error!")
